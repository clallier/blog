...
<Generation class="h-full max-h-96 w-full" />

::Card{color="note" v-click="2"}
...
<Steps :steps="[
  ...
  },
  {
    title: 'Alignment',
    icon: '🤝',
    body: 'Aligning output style and safety with human preferences (RLHF, DPO) - Yields **Instruction-Tuned** (IT) models.',
    footer: [
  ...
  },
  {
    title: 'Prompting',
    icon: '🦜',
    body: '**Programming** using **In-Context Learning** for new tasks at inference time. No weight updates required.',
    footer: [
...
Simplest way to run/test locally: [lmstudio.ai](https://lmstudio.ai/), :badge[Ollama], :badge[llama.cpp]
...
::Card{title="PPL for Neural Networks" color="note" v-click="1" class="mt-4"}
...
::Card{color="important" v-click="2" class="my-8"}
...
::Card{title="Concerns could be mitigated by:" color="important"}
...
- :badge[Generative Theory] Autoregressively predicts the likelihood of the next token.
- :badge[Training Lifecycle] Scales from Pre-training (knowledge) to SFT (behavior) and Alignment (safety).
- :badge[Limits & Boundaries] Intrinsic constraints like hallucinations, static knowledge, and security vulnerabilities (jailbreaks).
